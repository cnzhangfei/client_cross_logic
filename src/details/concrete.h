...
private:
    ...
    template <class FunctionSign> struct excute<FunctionSign, method_set> { template <class U, U> struct type_check { };
    ...
    template <class FunctionSign> struct excute<FunctionSign, method_get> { template <class U, U> struct type_check { };
...
public:
    ...
    void for_each(void* context, Consume consumer) const {
        action_check::for_each(
            (imp_data_store*)instance, [context, consumer](const std::string& k, const std::string& v) {
                if(consumer) {
    ...
    void debug(const std::string& name) const {
        std::stringstream ss;
        for_each(NULL,
            [&ss](void*, const std::string& k, const std::string& v) { ss << k << " : " << v << "," << std::endl; });
