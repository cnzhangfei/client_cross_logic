...
namespace marshal {
    ...
    template <class Action, int Version> class concrete : public agent {
        template <class FunctionSign, int MethodID> struct excute;
    ...
    private:
        ...
        template <class FunctionSign> struct excute<FunctionSign, method_valid> {
        ...
        template <class FunctionSign> struct excute<FunctionSign, method_finished> {
        ...
        template <class FunctionSign> struct excute<FunctionSign, method_set> {
            ...
            template <class _1>
            static bool
            do_it(Action* inst, const std::string& k, const std::string& v, type_check<FunctionSign, &_1::set>*) {
                return inst->set(k, v);
            }
        ...
        template <class FunctionSign> struct excute<FunctionSign, method_get> {

            template <class U, U> struct type_check {
            };

            template <class _1>
            static const std::string& do_it(Action* inst, const std::string& k, type_check<FunctionSign, &_1::get>*) {
                return inst->get(k);
            }
        ...
        template <class FunctionSign> struct excute<FunctionSign, method_remove> {

            template <class U, U> struct type_check {
            };

            template <class _1>
            static const std::string do_it(Action* inst, const std::string& k, type_check<FunctionSign, &_1::remove>*) {
    ...
    public:
        ...
        std::string remove(const std::string& key) {
            typedef excute<const std::string(Action::*)(const std::string&, const std::string&), method_remove>
                    remove_method;
        ...
        void for_each(void* context, Consume consumer) const {
            imp_data_store* imp = (imp_data_store*) instance;
            action_check::for_each([imp, context, consumer](const std::string& k, const std::string & v) {
                if (!consumer) {
                    return;
                }
                const std::string& value = imp->get(k);
                consumer(context, k, value);
            });
        }
        ...
        void debug(const std::string& name) const {
            std::stringstream ss;
            for_each(NULL,
                    [&ss](void*, const std::string& k, const std::string & v) {
                        ss << k << " : " << v << "," << std::endl; });
